...
class XglcdFont(object):
    ...
    Modified by Jeff Shepherd ([JAS]) at Eastern Washington University to support Python-native font data source files,
        in order to allow for freezing fonts as part of a MicroPython firmware file
    ...
    Note:
        ...
        The font file must be in X-GLCD 'C' format. [JAS] or Python files containing the same font data
    ...
    #def __init__(self, path, width, height, start_letter=32, letter_count=96, load_from_path=1):
    def __init__(self, path, width, height, start_letter=32, letter_count=96, is_freezable_font=False):
        """Constructor for X-GLCD Font object.

        Args:
            path (string): Full path of font file [JAS] if font data not preloaded by a subclass
            ...
            is_freezable_font (bool): [JAS] enable/disable load from filesystem (allows for freezing of subclass files)
        """
        self.width = width
        #self.height = height
        ...
        #if load_from_path == 1:
        if is_freezable_font == False:
            self.__load_xglcd_font(path)
        else:
            self.letters = bytearray(self.bytes_per_letter * self.letter_count) # prereserve space for the font data, to be filled by subclass
